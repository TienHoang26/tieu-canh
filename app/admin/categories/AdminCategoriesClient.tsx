            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input className="input flex-1" value={form.image_url}
                    onChange={e => setForm(f => ({ ...f, image_url: e.target.value }))}
                    placeholder="https://... hoặc upload từ máy" />
                  <label className={`flex items-center gap-2 px-4 py-2 rounded-xl border border-stone-200 text-sm font-medium cursor-pointer transition-colors shrink-0 ${uploading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-stone-50 text-stone-600'}`}>
                ...
                {form.image_url && (
                  <img src={form.image_url} alt=""
                    className="mt-2 h-24 w-full object-cover rounded-xl border border-stone-100" />
                )}
              </div>
